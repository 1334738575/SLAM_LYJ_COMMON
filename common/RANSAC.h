...
//	};
//template<>
//class RANSAC<Eigen::Vector3f, float, SLAM_LYJ::Line3f>
//{
//public:
//	typedef std::function<bool(const SLAM_LYJ::Line3f& mdl, const Eigen::Vector3f& data, float& err)> FuncCalError;
//	typedef std::function<bool(const std::vector<const Eigen::Vector3f*>& samples, SLAM_LYJ::Line3f& mdl)> FuncCalModule;
//	//dropped
//	RANSAC(const double _inlineRatioTh, const double _dstInlineRatioTh, const double _stopRatio,
//		const int _maxIterNum, const int _minInlineNum, FuncCalError _funcCalErr, FuncCalModule _funcCalModule)
//		:m_stopRatio(_stopRatio), m_minInlineNum(_minInlineNum), m_funcCalErr(_funcCalErr), m_funcCalModule(_funcCalModule)
...
//	~RANSAC() {};
//
//	double run(const std::vector<Eigen::Vector3f>& _datas, std::vector<float>& _errs, std::vector<bool>& _bInlines, SLAM_LYJ::Line3f& _mdl) {
//		const int dataSize = (int)_datas.size();
//		if (dataSize < m_minInlineNum)
//			return 0.0;
//		double bestRatio = m_stopRatio / 2;
...
//			if (inds[0] == -1)
//				continue;
//			std::vector<float> errs(dataSize);
//			std::vector<bool> bInlines(dataSize, false);
//			SLAM_LYJ::Line3f mdl;
//			double rat = runOnce(_datas, inds, errs, bInlines, mdl);
//			if (rat <= bestRatio)
...
//		randIndexGroup(_allSize, _eveSize, m_maxIterNum, _allSamples);
//	}
//	double runOnce(const std::vector<Eigen::Vector3f>& _datas, const std::vector<int>& _inds, std::vector<float>& _errs, std::vector<bool>& _bInlines, SLAM_LYJ::Line3f& _mdl) {
//		std::vector<const Eigen::Vector3f*> samples;
//		for (const auto& ind : _inds) {
//			samples.push_back(&_datas[ind]);
//		}
//		if (!m_funcCalModule(samples, _mdl)) {
