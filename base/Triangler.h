...
namespace COMMON_LYJ
{
	...
	//                if ((uvTmp - _uvs[i]).squaredNorm() > m_option.inlineErrTh)
	//                {
	//                    return false;
	//                }
	//            }
	...
	//            return true;
	//        };
	//        COMMON_LYJ::RANSAC<int, TYPE, TemVec3> ransac(m_option.ransacPriorInlineRatio, m_option.ransacMinInlineNum, m_option.ransacMaxItrtNum);
	...
	struct TrianglerPoint3DOption
	{
		...
		int ransacMinInlineNum = 2;
		int ransacMaxItrtNum = 20;
		double inlineErrTh = 1;
	};
	class SLAM_LYJ_API TrianglerPoint3D
	{
	public:
		...
		bool runDirect(const std::vector<Eigen::Vector2d>& _uvs, const std::vector<Pose3D>& _Tcws, const std::vector<PinholeCamera>& _cams, Eigen::Vector3d& _Pw, TrianglerPoint3DOption _opt = TrianglerPoint3DOption());
		bool runRANSAC(const std::vector<Eigen::Vector2d>& _uvs, const std::vector<Pose3D>& _Tcws, const std::vector<PinholeCamera>& _cams, Eigen::Vector3d& _Pw, TrianglerPoint3DOption _opt = TrianglerPoint3DOption());

	private:

	};

	struct TrianglerLine3DOption
	{
		double ransacPriorInlineRatio = 0.5;
		double ransacDstRatioTh = 0.99;
		double minRatio = 0.6;
		int ransacMinInlineNum = 2;
		int ransacMaxItrtNum = 20;
		double inlineErrTh = 2;
	};
	class SLAM_LYJ_API TrianglerLine3D
	{
	public:
		TrianglerLine3D() {};
		~TrianglerLine3D() {};

		bool runRANSAC(const std::vector<Eigen::Vector4d>& _l2ds, const std::vector<Pose3D>& _Tcws, const std::vector<PinholeCamera>& _cams, Eigen::Matrix<double, 6, 1>& _plk, TrianglerLine3DOption _opt = TrianglerLine3DOption());
	private:
